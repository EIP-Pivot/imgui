...
//  [x] Platform: Multi-viewport / platform windows. With issues (flickering
...
//  ImGuiPlatformIO interface. 2021-10-15: Vulkan: Call vkCmdSetScissor() at the
//  end of render a full-viewport to reduce likehood of issues with people using
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n++)
    if (ImGui_ImplVulkan_ViewportData *vd =
            (ImGui_ImplVulkan_ViewportData *)platform_io.Viewports[n]
                ->RendererUserData)
      ImGui_ImplVulkanH_DestroyWindowRenderBuffers(device, &vd->RenderBuffers,
                                                   allocator);
}
...
static void ImGui_ImplVulkan_DestroyWindow(ImGuiViewport *viewport) {
  ...
  // RendererUserData == NULL since we didn't create the data for it.
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  if (ImGui_ImplVulkan_ViewportData *vd =
    ...
    if (vd->WindowOwned)
      ImGui_ImplVulkanH_DestroyWindow(v->Instance, v->Device, &vd->Window,
                                      v->Allocator);
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, &vd->RenderBuffers,
                                                 v->Allocator);
    IM_DELETE(vd);
  }
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport *viewport,
  ...
  ImGui_ImplVulkan_ViewportData *vd =
      (ImGui_ImplVulkan_ViewportData *)viewport->RendererUserData;
  if (vd == NULL) // This is NULL for the main viewport (which is left to the
                  // user/app to handle)
    return;
  ImGui_ImplVulkan_InitInfo *v = &bd->VulkanInitInfo;
...
void ImGui_ImplVulkan_InitPlatformInterface() {
  ...
  if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    IM_ASSERT(platform_io.Platform_CreateVkSurface != NULL &&
